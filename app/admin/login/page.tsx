...
import { authenticateAdmin, checkAdminAuthentication } from "@/lib/storage-service"
...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAuthenticated) {
        // Salvar nome de usuário se "lembrar-me" estiver marcado
        ...
        // Pequeno atraso para garantir que o localStorage seja atualizado
        setTimeout(() => {
          router.push("/admin/dashboard")
        }, 500)
      } else {
    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-4 bg-black text-white relative overflow-hidden">
      ...
      <div className="max-w-md w-full z-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold mb-2 text-transparent bg-clip-text bg-gradient-to-r from-violet-400 via-fuchsia-500 to-indigo-600">
            Portal Onírico de Controle
          </h1>
          <p className="text-gray-300">Atravesse o portal para acessar o reino onírico do RugPullBoss</p>
        </div>
        ...
        <Card className="border-purple-800/30 bg-black/30 backdrop-blur-sm shadow-xl overflow-hidden">
          <CardHeader className="border-b border-purple-900/20 bg-black/50">
            <CardTitle className="text-xl text-fuchsia-400">Portal Dimensional</CardTitle>
            <CardDescription className="text-gray-400">Acesso restrito aos Guardiões do Reino Onírico</CardDescription>
          </CardHeader>
          <form onSubmit={handleLogin}>
            ...
            <CardFooter className="border-t border-purple-900/20 p-4 bg-black/50">
              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-700 hover:to-fuchsia-700 transition-all duration-300 shadow-lg shadow-fuchsia-600/20"
                disabled={isLoading}
              >
                {isLoading ? (
                  <>
